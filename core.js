  ...
  ws.on("connection", function connection(ws) {
      ...
      if (!rooms.get(_inRoomName).roomClients.has(_inID)) {
        ...
        if (rooms.get(_inRoomName).roomClients.size === 1) {
          ...
          try {
            rooms
              .get(_inRoomName)
              .roomClients.get(rooms.get(_inRoomName).roomMasterWSID)
              .ws.send(FMEventEncode("OnClientConnectedEvent", wsid));
          } catch {}
        }
      }
      ...
      for (var i = 0; i < rooms.get(_inRoomName).roomClients.size; i++) {
        console.log(
          "** ROOM [" +
            ...
            "] " +
            "(" +
            rooms.get(_inRoomName).roomMasterWSID +
            ")" +
            " client[" +
            i +
            "]: " +
            _roomInfo_clients.next().value.wsid
        );
      }
    ...
    function SetRoomMasterWSID(_inRoomName, _inWSID) {
      ...
      rooms
        .get(_inRoomName)
        .roomClients.get(rooms.get(_inRoomName).roomMasterWSID)
        .ws.send(FMEventEncode("OnRoomClientsUpdated", roomClientsWSIDs));
      rooms
        .get(_inRoomName)
        .roomClients.get(rooms.get(_inRoomName).roomMasterWSID)
        .ws.send(FMEventEncode("OnRoomMasterRequestEvent", "registered"));
      console.log(
        "** ROOM [" + _inRoomName + "] " + "SetRoomMasterWSID: " + _inWSID
      );
    }
    //
          ...
          if (rooms.get(_roomName).roomClients.has(wsid)) {
            ...
            if (rooms.get(_roomName).roomClients.size === 0) {
              ...
              if (rooms.get(_roomName).roomMasterWSID === wsid) {
                ...
                console.log(
                  "** Sent OnClientDisconnectedEvent to roomMasterWSID: " +
                    rooms.get(_roomName).roomMasterWSID +
                    " | disconnected client wsid: " +
                    wsid
                );
                try {
                  rooms
                    .get(_roomName)
                    .roomClients.get(rooms.get(_roomName).roomMasterWSID)
                    .ws.send(FMEventEncode("OnClientDisconnectedEvent", wsid));
                } catch {}
              }
            }
          }
    ...
    ws.on("message", function incoming(data, isBinary) {
      ...
      if (isBinary === false) {
        ...
        if (message.includes("fmevent:")) {
          ...
          if (decodedType !== "ping" && decodedType !== "lping")
            console.log("-> Message [" + message + "]"); //ignore ping message...
            ...
            case "ping":
              ...
              ws.send(
                FMEventEncode(
                  ...
                  decodedValue +
                    "," +
                    (_isRoomMaster ? "roomMaster" : "roomClient")
                )
              );
              break;
            case "networkType":
              ...
              ws.send(
                FMEventEncode("OnJoinedLobbyEvent", clients.get(wsid).roomName)
              );
              console.log(
                "regRoom(waiting request): " +
                  wsid +
                  " networkType: " +
                  clients.get(wsid).networkType +
                  " | roomName: " +
                  clients.get(wsid).roomName
              );
              break;
            case "roomName":
            ...
            if (clients.get(wsid).roomName !== "Lobby") {
              ...
              switch (data[1]) {
                ...
                case 3: //send to target
                  ...
                  try {
                    if (clients.get(_wsid).roomName === myRoomName)
                      clients.get(_wsid).ws.send(data);
                  } catch {}
                  break;
              }
            }
